...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask = None, memories = None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), attn_mask = attn_mask, memories = memories)))
...
class VisionTransformerAdapter(nn.Module):
    ...
    def forward_xmer(self, x, attn_mask = None, memories = None):
        for ind, block in enumerate(self.blocks):
            layer_memories = memories[ind] if exists(memories) else None
            x = block(x, attn_mask = attn_mask, memories = layer_memories)
        return x
...
# @torch.no_grad()
# def _load_weights(model: VisionTransformerAdapter, checkpoint_path: str, prefix: str = ''):
...
#         VisionTransformerAdapter, variant, pretrained,
